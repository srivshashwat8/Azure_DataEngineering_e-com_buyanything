...
# MAGIC )
# MAGIC select distinct(ProductID), ProductCategory, ProductName, UnitPrice
# MAGIC from cte
# MAGIC where rnk = 1
...
select distinct(ProductID), ProductCategory, ProductName, UnitPrice
from cte
where rnk = 1 
""")
...
path = "abfss://gold@cfstorageadlsgen2.dfs.core.windows.net/dim_product"

if DeltaTable.isDeltaTable(spark, path):
         df_sink =spark.sql(
             """
            select Dim_Product_Key, ProductID, ProductCategory, ProductName, UnitPrice
            from delta.`abfss://gold@cfstorageadlsgen2.dfs.core.windows.net/dim_product`
    ...
    df_sink= spark.createDataFrame([], schema= "Dim_Product_Key int, ProductID int, ProductCategory string, ProductName string, UnitPrice decimal(18,2) ")
...
df1= df_src.join(df_sink, df_src["ProductID"] == df_sink["ProductID"], "left").select(df_src["ProductID"], df_src["ProductCategory"], df_src["ProductName"], df_src["UnitPrice"], df_sink["Dim_Product_Key"])
df1.display()
...
df_old= df1.filter(df1["Dim_Product_Key"].isNotNull())
df_old.display()
...
df_new= df1.filter(df1["Dim_Product_Key"].isNull())
df_new.display()
...
# MAGIC %md
# MAGIC ## Get Maximum Dim_Product_Key
...
if incremental_flag == '0':
    ...
    max_value = df_old.agg(F.max("Dim_Product_Key")).collect()[0][0]
...
df = df_new.withColumn("Dim_Product_Key", max_value + F.monotonically_increasing_id())
df.display()
...
if DeltaTable.isDeltaTable(spark, path):
    ...
    deltaTable.alias("t").merge(df.alias("s"), "t.Dim_Product_Key=s.Dim_Product_Key").whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
else:
...
# MAGIC %sql
# MAGIC select * from delta.`abfss://gold@cfstorageadlsgen2.dfs.core.windows.net/dim_product`
