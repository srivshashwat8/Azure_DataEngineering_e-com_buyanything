...
# MAGIC %md
# MAGIC ## Working on Region Dimension

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC select distinct(SalesRegion)
...
path = "abfss://gold@cfstorageadlsgen2.dfs.core.windows.net/dim_region"

if DeltaTable.isDeltaTable(spark, path):
         df_sink =spark.sql(
             """
            select Dim_Region_Key, SalesRegion
            from delta.`abfss://gold@cfstorageadlsgen2.dfs.core.windows.net/dim_region`
...
df1= df_src.join(df_sink, df_src["SalesRegion"] == df_sink["SalesRegion"], "left").select(df_src["SalesRegion"], df_sink["Dim_Region_Key"])
df1.display()
...
df_old= df1.filter(df1["Dim_Region_Key"].isNotNull())
df_old.display()
...
df_new= df1.filter(df1["Dim_Region_Key"].isNull())
df_new.display()
...
# MAGIC %md
# MAGIC ## Get Maximum Dim_Region_Key
...
if incremental_flag == '0':
    ...
    max_value = df_old.agg(F.max("Dim_Region_Key")).collect()[0][0]
...
df = df_new.withColumn("Dim_Region_Key", max_value + F.monotonically_increasing_id())
df.display()
...
if DeltaTable.isDeltaTable(spark, path):
    ...
    deltaTable.alias("t").merge(df.alias("s"), "t.Dim_Region_Key=s.Dim_Region_Key").whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
else:
...
# MAGIC %sql
# MAGIC select * from delta.`abfss://gold@cfstorageadlsgen2.dfs.core.windows.net/dim_region`
